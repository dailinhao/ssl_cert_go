...
// 确保 DatabaseManager 实现了 DatabaseInterface 接口
var _ DatabaseInterface = &DatabaseManager{}
...
// SaveDomain 保存域名配置
func (dm *DatabaseManager) SaveDomain(domain *DomainConfig) error {
	// 序列化复杂字段
...
// GetDomain 获取域名配置
func (dm *DatabaseManager) GetDomain(id string) (*DomainConfig, error) {
	row := dm.db.QueryRow(`
	SELECT 
		id, domain, sub_domains, dns_provider, deployments, ssl_config, 
		auto_renew, renew_days, purchase_cert, cert_type, business, 
...
func (dm *DatabaseManager) GetAllDomains() ([]*DomainConfig, error) {
...
// DeleteDomain 删除域名配置
func (dm *DatabaseManager) DeleteDomain(id string) error {
	_, err := dm.db.Exec("DELETE FROM domains WHERE id = ?", id)
	return err
}

// SaveServer 保存服务器配置
func (dm *DatabaseManager) SaveServer(server *ServerConfig) error {
	// 序列化复杂字段
...
// GetServer 获取服务器配置
func (dm *DatabaseManager) GetServer(id string) (*ServerConfig, error) {
	row := dm.db.QueryRow(`
	SELECT 
...
func (dm *DatabaseManager) GetAllServers() ([]*ServerConfig, error) {
	rows, err := dm.db.Query(`
	SELECT 
		id, name, host, port, user, auth_type, password, ssh_key, tags, metadata 
	FROM servers
...
func (dm *DatabaseManager) DeleteServer(id string) error {
	_, err := dm.db.Exec("DELETE FROM servers WHERE id = ?", id)
	return err
}

// SaveAccount 保存账户配置
func (dm *DatabaseManager) SaveAccount(account *AccountConfig) error {
	// 序列化复杂字段
...
// GetAccount 获取账户配置
func (dm *DatabaseManager) GetAccount(id string) (*AccountConfig, error) {
	row := dm.db.QueryRow(`
	SELECT 
...
func (dm *DatabaseManager) GetAllAccounts() ([]*AccountConfig, error) {
	rows, err := dm.db.Query(`
	SELECT 
		id, name, cloud_vendor, region, access_key, secret_key, enabled, tags, metadata 
	FROM accounts
...
func (dm *DatabaseManager) DeleteAccount(id string) error {
	_, err := dm.db.Exec("DELETE FROM accounts WHERE id = ?", id)
	return err
}

// SaveUser 保存用户配置
func (dm *DatabaseManager) SaveUser(user *UserConfig) error {
	// 序列化复杂字段
...
// GetUser 获取用户配置
func (dm *DatabaseManager) GetUser(id string) (*UserConfig, error) {
	row := dm.db.QueryRow(`
	SELECT 
...
func (dm *DatabaseManager) GetAllUsers() ([]*UserConfig, error) {
	rows, err := dm.db.Query(`
	SELECT 
		id, username, password, email, role, permissions, enabled 
	FROM users
...
func (dm *DatabaseManager) SaveCertificate(cert *Certificate) error {
	// 序列化复杂字段
...
func (dm *DatabaseManager) GetCertificate(id string) (*Certificate, error) {
	row := dm.db.QueryRow(`
	SELECT 
...
func (dm *DatabaseManager) GetAllCertificates() ([]*Certificate, error) {
	rows, err := dm.db.Query(`
	SELECT 
		id, domain_id, domain, cert_pem, key_pem, issuer, not_before, not_after, serial_number, status, deployed_to 
	FROM certificates
...
func (dm *DatabaseManager) DeleteCertificate(id string) error {
	_, err := dm.db.Exec("DELETE FROM certificates WHERE id = ?", id)
	return err
}

// SaveCheckConfig 保存检查配置
func (dm *DatabaseManager) SaveCheckConfig(config *CheckConfig) error {
	// 序列化复杂字段
...
func (dm *DatabaseManager) GetCheckConfig() (*CheckConfig, error) {
	row := dm.db.QueryRow(`
	SELECT 
...
func (dm *DatabaseManager) MigrateFromFileSystem() error {
	// 加载文件系统中的域名配置
...
// 从文件系统加载域名配置
func loadDomainsFromFileSystem() ([]*DomainConfig, error) {
	// 获取当前可执行文件的目录
...
// 从文件系统加载服务器配置
func loadServersFromFileSystem() ([]*ServerConfig, error) {
	// 获取当前可执行文件的目录
	execDir, err := os.Getwd()
	if err != nil {
		return nil, err
...
// 从文件系统加载账户配置
func loadAccountsFromFileSystem() ([]*AccountConfig, error) {
	// 获取当前可执行文件的目录
	execDir, err := os.Getwd()
	if err != nil {
		return nil, err
	...
	for _, file := range files {
		...
		if filepath.Ext(file.Name()) == ".yaml" {
			filePath := filepath.Join(accountDir, file.Name())
			data, err := os.ReadFile(filePath)
			if err != nil {
				return nil, err
			}
...
// 从文件系统加载用户配置
func loadUsersFromFileSystem() ([]*UserConfig, error) {
	// 获取当前可执行文件的目录
	execDir, err := os.Getwd()
	if err != nil {
		return nil, err
...
// 从文件系统加载检查配置
func loadCheckConfigFromFileSystem() (*CheckConfig, error) {
	// 获取当前可执行文件的目录
	execDir, err := os.Getwd()
	if err != nil {
		return nil, err
	...
	// 检查文件是否存在
	if _, err := os.Stat(checkConfigFile); os.IsNotExist(err) {
		// 配置文件不存在，返回默认配置
		return &CheckConfig{
			Enabled:  true,
			Interval: "24h",
